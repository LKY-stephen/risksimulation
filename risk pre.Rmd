...
financialtrans=matrix(c(1,0.75,0.5,0.75,1,0.75,0.5,0.75,1),nrow=3)
...
disaster=function(states){
  penv=300/states[1,]
  pfin=150/states[2,]
  answer=-0.1*trying(rbind(penv,pfin))
...
Actions=function(choosing){
  ...
  if(max(actions[,,1])>0){
    result=result+invest_industry(actions[,,1])
  }
  if(max(actions[,,2]>0)){
...
midtermreport=function(){
  ...
  lines(envresult$c,type='b',col="cadetblue1")
...
```{r}
Actions(c(3,2,1))
finantial_supporting(c(3,3),c(1,2),c(200,100))
midtermreport()
```
